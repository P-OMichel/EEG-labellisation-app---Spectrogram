...
from DL.src.models import specseg_cnn, unet_temporal, resattn_unet, conv2_temporal_head, resattn_unet_tcn, unet_freq_gate_attention
...
def main():
    ...
    x_spec_path = cfg["data"].get("x_spec_path", "X_Y_dataset/X_spec_30_03_2026.npy")
    ...
    # NOTE: applied after normalization as in real use only unfiltered data will be used
    # Apply 3x3 median filter to each training spectrogram
    X_train_aug = np.stack([median_filter(x, size=(3, 3)) for x in X_train])
    # Concatenate original + augmented — labels are identical (same sample, same y)
