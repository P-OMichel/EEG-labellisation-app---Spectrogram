# -*- coding: utf-8 -*-
"""
EEG labeling app + read-only model suggestion
Version dedicated to 1D saved models only.

Behavior:
- Loads a 1D .npy EEG signal
- Displays EEG, spectrogram, editable mask, and model-predicted mask
- Only shows model bundles whose folder name contains '1D'
- Uses the raw 1D signal window as model input
- Resamples model predictions to len(t_spec) for display
- Autosaves editable masks to JSON

Assumptions:
- Bundle loader exists at: DL.src.io.bundle.load_bundle
- Saved 1D models output logits shaped (B,C,T) or (B,T,C)
- stats contain mean/std for normalization
"""
...
class AppConfig:
    ...
    model_name_token: str = "1D"

    default_bundle: Optional[str] = None
    device: str = "cuda"   # fixed typo from "cud"

    # Spectrogram settings for display only
    f_cut_hz: float = 45.0
...
# ---------------------------
# Bundle predictor for 1D models only
# ---------------------------
...
class DLBundledPredictor1D:
    ...
    def _load(self):
        try:
            from DL.src.io.bundle import load_bundle
        ...
        if stats is None:
            raise ValueError("load_bundle(...) did not return stats")
    ...
    def predict_mask_from_signal(self, sig: np.ndarray, out_T: int) -> np.ndarray:
        ...
        xt = torch.from_numpy(x).float().unsqueeze(0).unsqueeze(0).to(self._device)  # (1,1,T)
        ...
        if y.shape[1] == self.bundle.num_classes:
            ...
            raise ValueError(
                f"Cannot infer class dimension from output shape {tuple(y.shape)} "
...
# Main app
# ---------------------------
class EEGLabelerWithModel1D(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EEG labeling + 1D model suggestion (read-only)")
        ...
        self.predictor: Optional[DLBundledPredictor1D] = None
    ...
    def _build_ui(self):
        ...
        controls_2.addWidget(QtWidgets.QLabel("1D model bundle:"))
        ...
        self.plot_spec = pg.PlotWidget(title="Spectrogram (display only)")
        ...
        self.plot_model_mask = pg.PlotWidget(title="1D model mask (read-only suggestion)")
    ...
    def _browse_bundle(self):
        ...
        path = QtWidgets.QFileDialog.getExistingDirectory(self, "Select 1D model bundle folder", start_dir)
        if not path:
            return
    ...
    def _try_load_model(self, auto: bool = False, bundle_override: Optional[str] = None):
        ...
        if not bundle_dir:
            if not auto:
                self.lbl_model.setText("model: no 1D bundle selected")
            self.predictor = None
            self._model_ok = False
            return

        try:
            self.predictor = DLBundledPredictor1D(bundle_dir, device_pref=CFG.device)
            self._model_ok = True
            b = self.predictor.bundle
            if b is not None:
                self.lbl_model.setText(
                    f"model: OK ({os.path.basename(bundle_dir)} | {b.arch} | raw 1D signal | dev={self.predictor.device.type})"
                )
            else:
    ...
    def _render_current_window(self):
        ...
        if self.predictor is not None and len(self.t_spec) > 0:
            try:
                y_pred = self.predictor.predict_mask_from_signal(self.sig_win, out_T=len(self.t_spec))
