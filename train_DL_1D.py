...
from DL.src.datasets.spectrogram_dataset import TimeSeriesSegDataset, compute_norm_stats, apply_norm
from DL.src.models.registry import build_model
from DL.src.io.bundle import save_bundle
from DL.src.training.trainer_1D import fit
...
def save_test_confusion_matrix(
    ...
    for xb, yb in test_loader:
        ...
        yb = yb.to(device)
        if xb.ndim != 3:
            raise ValueError(f"Expected xb with 4 dims (B,1,L), got {tuple(xb.shape)}")

        if xb.shape[1] != 1:
            raise ValueError(f"Expected xb shape (B,1,L), got {tuple(xb.shape)}")
...
def main():
    ...
    train_ds = TimeSeriesSegDataset(X_train, y_train)
    val_ds   = TimeSeriesSegDataset(X_val, y_val)
    test_ds  = TimeSeriesSegDataset(X_test, y_test)  
