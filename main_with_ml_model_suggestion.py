...
class AppConfig:
    ...
    # XGBoost model path (json/bst)
    model_path: str = "model_10_classes_lags.json"
    # Optional scaler (joblib)
    scaler_path: Optional[str] = None  # e.g. "robust_scaler.joblib"

    # If you trained with lags=(1,2), keep this
    lags: tuple[int, ...] = (1, 2)

    # Spectrogram settings (keep consistent with training)
    f_cut_hz: float = 45.0
...
# ---------------------------
# Model inference: lagging + features + predictor
# ---------------------------
def _add_lags(X: np.ndarray, lags: tuple[int, ...]) -> np.ndarray:
    if not lags:
        return X
    T, D = X.shape
    cols = [X]
...
def extract_features_for_window(eeg_win: np.ndarray, fs: float, f_spec: np.ndarray, t_spec: np.ndarray, Sxx: np.ndarray) -> np.ndarray:
    T = len(t_spec)
    if T == 0:
    ...
    mean, med_mean, q_std, med_q_std, q_linelen, med_q_linelen, q_env, med_q_env, q_std_env, med_q_std_env = \
        extract_features_time_series(eeg_win, int(fs / 8), y_dummy)

    ef, ef_recovery, med_ef, med_ef_recovery, prop_delta, prop_alpha, prop_beta, prop_gamma, q_P_tot, med_q_P_tot, slopes = \
        extract_features_spectrogram(np.array(f_spec), Sxx)

    X_list = [
        mean, med_mean, q_std, med_q_std, q_linelen, med_q_linelen,
        q_env, med_q_env, q_std_env, med_q_std_env,
        ef, ef_recovery, med_ef, med_ef_recovery,
        prop_delta, prop_alpha, prop_beta, prop_gamma,
        q_P_tot, med_q_P_tot, slopes
    ]
    X0 = np.vstack([np.asarray(f).ravel() for f in X_list]).T
    X = _add_lags(X0, CFG.lags)
    X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
    return X.astype(float)


class XGBMaskPredictor:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        X2 = self.scaler.transform(X) if self.scaler is not None else X
        dmat = xgb.DMatrix(X2)
...
class EEGLabelerWithModel(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.predictor: Optional[XGBMaskPredictor] = None
    ...
    def _build_ui(self):
        ...
        # model status + reload
        controls.addSpacing(16)
        self.btn_reload_model = QtWidgets.QPushButton("Reload model")
    ...
    def _try_load_model(self):
        try:
            self.predictor = XGBMaskPredictor(CFG.model_path, CFG.scaler_path)
            self._model_ok = True
            self.lbl_model.setText(f"model: OK ({os.path.basename(CFG.model_path)})")
    ...
    def _render_current_window(self):
        ...
        if self.predictor is not None and len(self.t_spec) > 0:
            try:
                X = extract_features_for_window(self.sig_win, self.fs, self.f, self.t_spec, self.Sxx)
    ...
    def _update_plots(self):
        ...
        self.plot_model_mask.setYRange(-0.5, MASK_MAX + 0.5, padding=0.02)
